...
export function initTools(data: CVData): void {
  ...
  // Extract individual languages from comma-separated combos
  const langSet = new Set<string>();
  tools.forEach((t) => {
    if (t.Language) {
      t.Language.split(',').map((l) => l.trim()).filter(Boolean).forEach((l) => langSet.add(l));
    }
  });
  ...
  function toolMatchesLang(tool: typeof tools[0], lang: string): boolean {
    if (!tool.Language) return false;
    return tool.Language.split(',').map((l) => l.trim()).includes(lang);
  }
  ...
  function render() {
    const filtered = activeFilter === 'All'
      ? tools
      : tools.filter((t) => toolMatchesLang(t, activeFilter));
      ...
      <div class="tools__filters">
        ${allFilters.map((f) => `
          <button class="tools__filter ${f === activeFilter ? 'tools__filter--active' : ''}" data-filter="${f}">
            ${f}${f !== 'All' ? ` (${tools.filter(t => toolMatchesLang(t, f)).length})` : ` (${tools.length})`}
          </button>
        `).join('')}
      </div>
      ...
      <div class="tools__grid ${compact ? 'tools__grid--compact' : ''}" style="--grid-cols: ${cols};">
        ${filtered.map((t) => `
          <div class="tool-card">
            ...
            ${t.Language ? `<div class="tool-card__langs">${t.Language.split(',').map(l => `<span class="tool-card__lang">${l.trim()}</span>`).join('')}</div>` : ''}
            <div class="tool-card__icons">
              ${t.Link ? `<a href="${t.Link}" class="tool-card__icon-link" target="_blank" rel="noopener noreferrer" title="Docs"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg></a>` : ''}
              ...
              ${t.GitHub ? `<a href="${t.GitHub}" class="tool-card__icon-link tool-card__icon-link--github" target="_blank" rel="noopener noreferrer" title="GitHub"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg></a>` : ''}
            </div>
          </div>
        `).join('')}
      </div>
